...
impl Interface for ConnectedInterface {
    ...
    fn on_selection(mut self: Box<Self>, choice: u32) -> Box<dyn Interface> {
        match choice {
            1 => {
                let response = self.server.transfer_request(ServerRequest::ListQueues).unwrap();
            ...
            2 => {
                ...
                let response = self.server.transfer_request(ServerRequest::CheckQueue(selection.clone())).unwrap();
            ...
            3 => {
                ...
                let response = self.server.transfer_request(ServerRequest::CreateQueue(name)).unwrap();
                println!("Response {:?}", response);
                Box::new(ConnectedInterface { server: self.server, selected_queue: self.selected_queue })
            }
            4 => {
                if let Some(queue) = &self.selected_queue {
                    let response = self.server.transfer_request(ServerRequest::MakeSender(queue.clone())).unwrap();
                    println!("Response {:?}", response);
                    self.send_messages();
                } else {
